...
class FoodViewSet(LoggingMixin, CustomViewSet):
    ...
    def update(self, request, pk, *args, **kwargs):
        ...
        if not restaurant_id:
            food_qs =  Food.objects.filter(id = pk).first()
            restaurant_id=  food_qs.restaurant_id
...
class InvoiceViewSet(LoggingMixin, CustomViewSet):
    ...
    def invoice_all_report(self, request=None, restaurant=None, getOnlyData=False, *args, **kwargs):
        ...
        total_service_charge = sum(pre_service_charge_amount)
        # Total Vat Amount
        pre_vat_amount = invoice_filter_qs.values_list(
            'order__tax_amount', flat=True
        )
        ...
        if getOnlyData == True:
            ...
            page_qs = self.paginate_queryset(invoice_filter_qs)
